...
abstract class ValidatePageSizeTask : DefaultTask() {
    private val ALLOWED_PAGE_SIZES =
        setOf(
            "2**14", // 16KB
            "2**15", // 32KB
            "2**16", // 64KB
        ...
        outDirs.asFileTree.files
            ...
            .forEach {
                ...
                if (alignment in ALLOWED_PAGE_SIZES) {
                    ...
                    val message =
                        "Expected ELF alignment of 2**14 or higher for file `${it.name}`, " +
                            "but got $alignment.\n Please see " +
...
internal fun Project.configurePageSizeCheck() {
    val objDumpFile =
        try {
            val androidNdkHome =
                componentsExtension.sdkComponents.ndkDirectory
                    .get()
                    .asFile
...
private fun Task.getDummyOutput(): Provider<RegularFile> = project.layout.buildDirectory.file("dummyOutput/" + this.name.replace(":", "-"))
